...
<table class="full-width border-box mt-10 mb-10">
    ...
    <tr>
        ...
        <th class="border-top-bottom text-center">Observación</th>
        <th class="border-top-bottom text-center">Unidad</th>
        <th class="border-top-bottom text-center">Precio Unitario</th>
    ...
    @foreach($document->items as $row)
        <tr>
            ...
            <td class="text-center">
                @isset($row->item->second_name)
                    {{ $row->item->second_name }}
                @endisset
            </td>
            <td class="text-center">{{ $row->item->unit_type_id }}</td>
            <td class="text-center">
                @isset($row->item->unit_price)
                    {{ $row->item->unit_price }}
                @endisset
            </td>
            <td class="text-right">
