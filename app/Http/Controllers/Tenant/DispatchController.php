...
class DispatchController extends Controller
{
    ...
    public function store(DispatchRequest $request)
    {
        ...
        try {
            // Configuración dinámica de la conexión a SQL Server
            $dbConfig = Company::first();
            config([
                'database.connections.dynamic_sqlsrv' => [
                    'driver' => $dbConfig->server_driver,
                    'url' => $config->server_url,
                    'host' => $dbConfig->server_host,
                    'port' => $dbConfig->server_port,
                    'database' => $dbConfig->server_database,
                    'username' => $dbConfig->server_username,
                    'password' => $dbConfig->server_password,
                    'charset' => $dbConfig->server_charset,
                    'prefix' => $dbConfig->server_prefix,
                    'prefix_indexes' => $dbConfig->server_prefix_indexes,
                ],
            ]);
    
            // Datos para insertar en CHTEmisionCab
            $emisionCabData = [
                'IdTipoDocumento' => $document->type,
                'SerieDocumento' => $document->series,
                'NumeroDocumento' => $document->number,
                'FechaEmision' => $document->date_of_issue,
                'IdOficinas' => $document->establishment_id, // Asumiendo que este campo es IdOficinas
                'IdDestino' => $document->delivery_address_id, // Asumiendo que este campo es IdDestino
                'RemitenteRuc' => $document->customer_id, // Asumiendo que este campo es RemitenteRuc
                'RemitenteNombre' => $document->customer->name, // Asumiendo que este campo es RemitenteNombre
                'RemitenteDireccion' => $document->customer->address, // Asumiendo que este campo es RemitenteDireccion
                'RemitenteCiudad' => $document->customer->city, // Asumiendo que este campo es RemitenteCiudad
                'ConsignatarioRuc' => $document->receiver_data['ruc'], // Asumiendo que este campo es ConsignatarioRuc
                'ConsignatarioNombre' => $document->receiver_data['name'], // Asumiendo que este campo es ConsignatarioNombre
                'ConsignatarioDireccion' => $document->receiver_data['address'], // Asumiendo que este campo es ConsignatarioDireccion
                'ConsignatarioCiudad' => $document->receiver_data['city'], // Asumiendo que este campo es ConsignatarioCiudad
                'LugarEntrega' => $document->delivery['address'], // Asumiendo que este campo es LugarEntrega
                'Referencia' => $document->observations, // Asumiendo que este campo es Referencia
                'IdEstado' => 0, // Valor por defecto
                'IdPersonal' => $document->driver_id, // Asumiendo que este campo es IdPersonal
                'FechaActividad' => now(), // Asumiendo que este campo es FechaActividad
                'IdEmpresa' => $company->id, // Asumiendo que este campo es IdEmpresa
                'CheckDocumento' => 1, // Valor por defecto
                'Periodo' => date('Ym'), // Asumiendo que este campo es Periodo
            ];
    
            // Insertar datos en la tabla CHTEmisionCab
            DB::connection('dynamic_sqlsrv')->table('CHTEmisionCab')->insert($emisionCabData);
    
            // Insertar datos en la tabla CHTEmisionDet
            foreach ($document->items as $item) {
                $emisionDetData = [
                    'IdEmisionCab' => $document->id, // ID de la emisión cabecera
                    'ItemId' => $item['id'], // ID del item
                    'ItemDescription' => $item['description'], // Descripción del item
                    'ItemQuantity' => $item['quantity'], // Cantidad del item
                    // Agrega aquí los demás campos necesarios para CHTEmisionDet
                ];
    
                DB::connection('dynamic_sqlsrv')->table('CHTEmisionDet')->insert($emisionDetData);
            }
        } catch (\Exception $e) {
            // Manejo del error, por ejemplo, registrar el error en los logs
        }
